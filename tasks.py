...
import config
from protenix_result_utils import (
    ProtenixResultError,
    build_protenix_confidence_payload,
    collect_confidence_issues,
    extract_structure_confidence,
    find_ligand_chain_and_resname,
    resolve_protenix_best_structure,
)
...
def _split_chain_tokens(raw_chain_value: str | None) -> list[str]:
    if not raw_chain_value:
        return []
    return [token.strip().upper() for token in str(raw_chain_value).split(",") if token.strip()]


def _validate_protenix_chain_consistency(
    *,
    input_structure_path: str,
    structure_confidence: dict[str, Any],
    target_chain: str | None = None,
) -> None:
    """Fail fast when Protenix output chains are inconsistent with input component topology."""
    input_protein_chain_ids = _extract_protein_chain_ids_from_structure(input_structure_path)
    if not input_protein_chain_ids:
        return

    output_polymer_raw = structure_confidence.get("polymer_chain_ids")
    output_polymer_chain_ids = [
        str(item).strip()
        for item in output_polymer_raw
        if isinstance(item, str) and str(item).strip()
    ] if isinstance(output_polymer_raw, list) else []
    if not output_polymer_chain_ids:
        return

    target_chain_tokens = _split_chain_tokens(target_chain)
    has_single_target = len(target_chain_tokens) <= 1
    has_single_input_protein_chain = len(input_protein_chain_ids) == 1

    # For single-protein-chain tasks, output should not expand into extra polymer chains.
    if has_single_target and has_single_input_protein_chain and len(output_polymer_chain_ids) > 1:
        raise RuntimeError(
            "Protenix output chain over-segmentation detected: "
            f"input protein chains={input_protein_chain_ids}, "
            f"output polymer chains={output_polymer_chain_ids}. "
            "This indicates a malformed intermediate structure conversion."
        )


def _run_protenix_affinity_with_boltzina(
    *,
    structure_path: Path,
    task_temp_dir: str,
    preferred_ligand_chain: str | None,
) -> tuple[dict[str, Any] | None, Path | None, Path | None]:
    """Run Boltzina affinity on the selected Protenix structure."""
    ligand_info = find_ligand_chain_and_resname(structure_path, preferred_ligand_chain)
    if not ligand_info:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to import Boltzina affinity module: {exc}") from exc

    binder_chain, ligand_resname = ligand_info
    affinity_root = Path(task_temp_dir) / "protenix_affinity"
    affinity_output_dir = affinity_root / "boltzina_output"
    affinity_work_dir = affinity_root / "boltzina_work"
    affinity_output_dir.mkdir(parents=True, exist_ok=True)
    affinity_work_dir.mkdir(parents=True, exist_ok=True)
    ...
    boltzina.predict([str(structure_path)])
    if not boltzina.results:
        return None, None, affinity_output_dir

    affinity_payload = dict(boltzina.results[0])
    affinity_payload["source"] = "protenix"
    affinity_payload["binder_chain"] = binder_chain
    affinity_payload["ligand_resname"] = ligand_resname
    affinity_csv_path = affinity_output_dir / "affinity_results.csv"
    return affinity_payload, (affinity_csv_path if affinity_csv_path.exists() else None), affinity_output_dir
...
@celery_app.task(bind=True)
def protenix2score_task(self, score_args: dict):
    """
    Celery task for running Protenix2Score (confidence + optional Boltzina affinity).
    """
    gpu_id = -1
    task_id = self.request.id
    task_temp_dir = None
    ...
    try:
        ...
        tracker = TaskProgressTracker(task_id, redis_client)
        tracker.start_heartbeat()
        tracker.update_status("starting", "Initializing Protenix2Score task")

        logger.info(f"Task {task_id}: Attempting to acquire GPU for Protenix2Score.")
        tracker.update_status("acquiring_gpu", "Waiting for GPU allocation")
        gpu_id = acquire_gpu(task_id=task_id, timeout=3600)
        self.update_state(state='PROGRESS', meta={'status': f'Acquired GPU {gpu_id}. Starting Protenix2Score.'})
        tracker.update_status("gpu_acquired", f"Using GPU {gpu_id}")

        task_temp_dir = tempfile.mkdtemp(prefix=f"protenix2score_task_{task_id}_")
        input_filename = secure_filename(score_args['input_filename'])
        input_file_path = os.path.join(task_temp_dir, input_filename)
        with open(input_file_path, 'w', encoding='utf-8') as f:
            f.write(score_args['input_file_content'])
        ...
        command = [
            sys.executable,
            str(BASE_DIR / "Protenix2Score" / "protenix2score.py"),
        ...
        tracker.update_status("running", "Executing Protenix2Score subprocess")
        logger.info(f"Task {task_id}: Running Protenix2Score. Command: {' '.join(command)}")
        ...
        try:
            ...
            stdout, stderr = process.communicate()
            error_message = (
                f"Subprocess for Protenix2Score task {task_id} timed out after {SUBPROCESS_TIMEOUT} seconds.\n"
                f"Stderr (tail):\n{_truncate_text(stderr, MAX_STDIO_TAIL_CHARS, prefer_tail=True)}\n"
                f"Stdout (tail):\n{_truncate_text(stdout, MAX_STDIO_TAIL_CHARS, prefer_tail=True)}"
            )
            logger.error(error_message)
        ...
        if process.returncode != 0:
            error_message = _format_subprocess_failure("Protenix2Score task", task_id, process.returncode, stderr, stdout)
            logger.error(error_message)
            tracker.update_status("failed", f"Process failed with exit code {process.returncode}")
            raise RuntimeError(error_message)

        tracker.update_status("processing_output", "Packaging Protenix2Score results")
        metrics_path = os.path.join(output_dir, "protenix2score_metrics.json")
        metrics_payload = {}
        if os.path.exists(metrics_path):
            try:
                ...
                logger.warning("Task %s: failed to parse Protenix2Score metrics: %s", task_id, e)
        if not isinstance(metrics_payload, dict) or not metrics_payload:
            raise RuntimeError(
                "Protenix2Score completed but protenix2score_metrics.json is missing or invalid."
            )
        best_summary = metrics_payload.get("best_summary")
        if not isinstance(best_summary, dict):
            raise RuntimeError(
                "Protenix2Score metrics missing best_summary; cannot determine canonical confidence payload."
            )

        target_chain = str(score_args.get("target_chain") or "").strip()
        ligand_chain = str(score_args.get("ligand_chain") or "").strip()

        try:
            best_structure_path = resolve_protenix_best_structure(output_dir, metrics_payload)
        except ProtenixResultError as exc:
            raise RuntimeError(f"Failed to resolve Protenix best structure: {exc}") from exc

        structure_ext = best_structure_path.suffix.lower()
        if structure_ext == ".mmcif":
            structure_ext = ".cif"
        if structure_ext not in {".cif", ".pdb"}:
            structure_ext = ".cif"
        staged_structure = os.path.join(task_temp_dir, f"protenix_model_0{structure_ext}")
        shutil.copyfile(str(best_structure_path), staged_structure)
        try:
            structure_confidence = extract_structure_confidence(Path(staged_structure))
            _validate_protenix_chain_consistency(
                input_structure_path=input_file_path,
                structure_confidence=structure_confidence,
                target_chain=target_chain or None,
            )
            confidence_payload = build_protenix_confidence_payload(
                metrics_payload,
                structure_confidence=structure_confidence,
                preferred_ligand_chain=ligand_chain or None,
            )
        except ProtenixResultError as exc:
            raise RuntimeError(f"Failed to build Protenix confidence payload: {exc}") from exc

        ligand_smiles_map_raw = score_args.get("ligand_smiles_map")
        normalized_ligand_smiles_map: dict[str, str] = {}
        if isinstance(ligand_smiles_map_raw, dict) and ligand_smiles_map_raw:
            for key, value in ligand_smiles_map_raw.items():
                if not isinstance(key, str) or not isinstance(value, str):
                    continue
                key_norm = key.strip().upper()
                value_norm = value.strip()
                if key_norm and value_norm:
                    normalized_ligand_smiles_map[key_norm] = value_norm
            selected_ligand_smiles = ""
            if ligand_chain:
                selected_ligand_smiles = normalized_ligand_smiles_map.get(ligand_chain.strip().upper(), "")
            if not selected_ligand_smiles and len(normalized_ligand_smiles_map) == 1:
                selected_ligand_smiles = next(iter(normalized_ligand_smiles_map.values()))
            if selected_ligand_smiles:
                confidence_payload["ligand_smiles"] = selected_ligand_smiles

        for issue in collect_confidence_issues(metrics_payload, confidence_payload):
            logger.warning("Task %s: Protenix confidence issue: %s", task_id, issue)

        enable_affinity = bool(score_args.get("enable_affinity"))
        auto_enable_affinity = bool(score_args.get("auto_enable_affinity"))
        wants_affinity = enable_affinity or auto_enable_affinity
        if enable_affinity and (not target_chain or not ligand_chain):
            raise ValueError(
                "Affinity requested for Protenix2Score but target_chain/ligand_chain is missing."
            )
        run_affinity = wants_affinity and bool(target_chain and ligand_chain)
        affinity_payload: dict[str, Any] | None = None
        affinity_csv_path: Path | None = None
        affinity_artifacts_dir: Path | None = None
        if run_affinity:
            tracker.update_status("running_affinity", "Running Boltzina affinity on Protenix model")
            previous_cuda_visible = os.environ.get("CUDA_VISIBLE_DEVICES")
            os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
            try:
                affinity_payload, affinity_csv_path, affinity_artifacts_dir = _run_protenix_affinity_with_boltzina(
                    structure_path=Path(staged_structure),
                    task_temp_dir=task_temp_dir,
                    preferred_ligand_chain=ligand_chain,
                )
            finally:
            ...
            if affinity_payload is None:
                message = "Boltzina affinity run did not produce a result for Protenix2Score."
                if enable_affinity:
                    raise RuntimeError(message)
                logger.warning("Task %s: %s", task_id, message)
            else:
                if target_chain:
                    affinity_payload["requested_target_chain"] = target_chain
                if ligand_chain:
                    affinity_payload["requested_ligand_chain"] = ligand_chain
                if normalized_ligand_smiles_map:
                    affinity_payload["ligand_smiles_map"] = dict(normalized_ligand_smiles_map)
                    selected_ligand_smiles = ""
                    if ligand_chain:
                        selected_ligand_smiles = normalized_ligand_smiles_map.get(ligand_chain.strip().upper(), "")
                    if not selected_ligand_smiles and len(normalized_ligand_smiles_map) == 1:
                        selected_ligand_smiles = next(iter(normalized_ligand_smiles_map.values()))
                    if selected_ligand_smiles:
                        affinity_payload["ligand_smiles"] = selected_ligand_smiles

        output_archive_path = os.path.join(task_temp_dir, f"{task_id}_results.zip")
        with zipfile.ZipFile(output_archive_path, 'w') as zipf:
            zipf.write(staged_structure, f"protenix/output/protenix_model_0{structure_ext}")
            zipf.writestr(
                "protenix/output/confidence_protenix_model_0.json",
                json.dumps(confidence_payload, ensure_ascii=False, indent=2),
            )
            zipf.writestr(
                "protenix/output/protenix2score_metrics.json",
                json.dumps(metrics_payload, ensure_ascii=False, indent=2),
            )
            if affinity_payload:
                zipf.writestr(
                    "protenix/output/affinity_data.json",
                    json.dumps(affinity_payload, ensure_ascii=False, indent=2),
                )
            if affinity_csv_path and affinity_csv_path.exists():
                zipf.write(str(affinity_csv_path), "protenix/output/affinity_results.csv")
            if affinity_artifacts_dir and affinity_artifacts_dir.exists():
                for root, _, files in os.walk(affinity_artifacts_dir):
                    for file in files:
                        file_path = os.path.join(root, file)
                        rel = os.path.relpath(file_path, affinity_artifacts_dir)
                        zipf.write(file_path, os.path.join("protenix/output/affinity_artifacts", rel))
            for log_name in ("protenix2score_tojson.log", "protenix2score_infer.log"):
                log_path = os.path.join(output_dir, log_name)
                if os.path.exists(log_path):
                    zipf.write(log_path, os.path.join("protenix/output", log_name))
        ...
        self.update_state(state='SUCCESS', meta=final_meta)
        tracker.update_status("completed", "Task completed successfully")
        logger.info(f"Task {task_id}: Protenix2Score completed and results uploaded successfully.")
        return final_meta
        ...
        if task_temp_dir and os.path.exists(task_temp_dir):
            try:
                ...
                logger.warning(
                    "Task %s: failed to clean temporary directory '%s': %s",
                    task_id,
                    task_temp_dir,
                    cleanup_err,
                )
