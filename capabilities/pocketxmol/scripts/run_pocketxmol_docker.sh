...
COMPOSE_FILE_DEFAULT="${PROJECT_ROOT}/deploy/docker/DOCKER_CAP_POCKETXMOL.compose.yml"
COMPOSE_FILE="${PXM_DOCKER_COMPOSE_FILE:-${COMPOSE_FILE_DEFAULT}}"
COMPOSE_ENV_FILE="${PXM_DOCKER_ENV_FILE:-}"
cd "${ROOT_DIR}"
...
if [[ ! -f "${COMPOSE_FILE}" ]]; then
  echo "PocketXMol compose file not found: ${COMPOSE_FILE}" >&2
  exit 1
fi

export PXM_HOST_WORKSPACE="${ROOT_DIR}"
DOCKERFILE_PATH="${PROJECT_ROOT}/deploy/docker/DOCKER_CAP_POCKETXMOL.Dockerfile"
...
run_pocketxmol_container() {
  ...
  local docker_cmd=(
    ...
    --device-cgroup-rule "c 510:* rmw"
    --device-cgroup-rule "c 235:* rmw"
    -e "NVIDIA_DRIVER_CAPABILITIES=compute,utility"
    -e "DOCKER_UID=0"
    -e "DOCKER_GID=0"
    -v "${PXM_HOST_WORKSPACE}:/workspace"
  )
  ...
  add_bind_mount() {
    ...
    if [[ -n "${mounted_paths["${source}"]:-}" ]]; then
      return
    fi
    mounted_paths["${source}"]=1
    docker_cmd+=(-v "${source}:${source}")
  }
  add_bind_mount "${PROJECT_ROOT}"
  add_bind_mount "/tmp"

  local results_base_dir="${RESULTS_BASE_DIR:-}"
  if [[ -n "${results_base_dir}" ]]; then
    mkdir -p "${results_base_dir}"
    add_bind_mount "${results_base_dir}"
  fi
  local config_task_path="${PXM_CONFIG_TASK:-}"
  if [[ -n "${config_task_path}" && "${config_task_path}" = /* ]]; then
    local config_task_mount="${config_task_path}"
    if [[ ! -d "${config_task_mount}" ]]; then
      config_task_mount="$(dirname "${config_task_mount}")"
    fi
    mkdir -p "${config_task_mount}"
    add_bind_mount "${config_task_mount}"
  fi
  local outdir_path="${PXM_OUTDIR:-}"
  if [[ -n "${outdir_path}" && "${outdir_path}" = /* ]]; then
    mkdir -p "${outdir_path}"
    add_bind_mount "${outdir_path}"
  fi
  if [[ -n "${PXM_NVIDIA_VISIBLE_DEVICES:-}" ]]; then
    docker_cmd+=(-e "NVIDIA_VISIBLE_DEVICES=${PXM_NVIDIA_VISIBLE_DEVICES}")
  else
    docker_cmd+=(-e "NVIDIA_VISIBLE_DEVICES=all")
  fi
  docker_cmd+=(
    -e "PXM_CONFIG_TASK=${PXM_CONFIG_TASK:-}"
    -e "PXM_CONFIG_MODEL=${PXM_CONFIG_MODEL:-}"
    -e "PXM_OUTDIR=${PXM_OUTDIR:-}"
    -e "PXM_DEVICE=${PXM_DEVICE:-}"
    -e "PXM_BATCH_SIZE=${PXM_BATCH_SIZE:-}"
  )
  if [[ -n "${PXM_EXTRA_ARGS:-}" ]]; then
    docker_cmd+=(-e "PXM_EXTRA_ARGS=${PXM_EXTRA_ARGS}")
  fi
  docker_cmd+=("${IMAGE_NAME}" "${command_args[@]}")
  "${docker_cmd[@]}"
}

load_env_file "${COMPOSE_ENV_FILE}"
IMAGE_NAME="${POCKETXMOL_DOCKER_IMAGE:-pocketxmol:cu128}"

if [[ "${DO_BUILD}" == "1" ]]; then
  echo "[run_pocketxmol_docker] docker build -t ${IMAGE_NAME}"
...
if [[ "${RESCORE_ONLY}" != "1" ]]; then
  ...
  echo "[run_pocketxmol_docker] Running inference via docker run"
  ...
  run_pocketxmol_container bash docker/run_inference.sh
fi
