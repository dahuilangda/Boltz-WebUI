...
This directory contains the runtime code that V-Bio currently uses for Lead Optimization.

> Path update notice:
> - `lead_optimization/...` in historical command examples should now be read as `capabilities/lead_optimization/...`.
> - `python -m lead_optimization...` should now be read as `python -m capabilities.lead_optimization...`.
...
Recommended: set these in your stack env file (for example `deploy/docker/DOCKER_STACK_WORKER_CPU.env`) before running API/Celery:
...
```env
LEAD_OPT_MMP_DB_REGISTRY=/data/V-Bio/lead_optimization/data/mmp_db_registry.json
```
...
Step 3: switch runtime to this schema (edit your backend stack env)
