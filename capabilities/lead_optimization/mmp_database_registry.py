...
def _parse_postgres_dsn_identity(dsn: str) -> Dict[str, Any]:
    token = str(dsn or "").strip()
    if not _is_postgres_dsn(token):
        return {}
    try:
        parsed = urlparse(token)
    except Exception:
    ...
    return {
        ...
        "port": int(parsed.port or _DEFAULT_PG_PORT),
...
def _should_canonicalize_localhost_dsn(candidate_dsn: str, configured_dsn: str) -> bool:
    candidate = _parse_postgres_dsn_identity(candidate_dsn)
    configured = _parse_postgres_dsn_identity(configured_dsn)
    if not candidate or not configured:
        return False
    if candidate["host"] not in _LOCALHOST_DSN_HOSTS:
        return False
    if not configured["host"] or configured["host"] in _LOCALHOST_DSN_HOSTS:
        return False
    if candidate["database"] != configured["database"]:
        return False
    if candidate["port"] != configured["port"]:
        return False
    if candidate["username"] != configured["username"]:
        return False
    candidate_password = candidate.get("password")
    configured_password = configured.get("password")
    if candidate_password is not None and configured_password is not None and candidate_password != configured_password:
        return False
    return True


def _canonicalize_manual_postgres_dsn(dsn: str) -> str:
    token = str(dsn or "").strip()
    if not token:
        return token
    configured_dsn, backend = _resolve_default_database()
    if backend != "postgres" or not _should_canonicalize_localhost_dsn(token, configured_dsn):
        return token
    return configured_dsn


def _connect_postgres(dsn: str, *, autocommit: bool = True):
    if psycopg is None:
        raise RuntimeError("psycopg is required for PostgreSQL operations.")
    primary = str(dsn or "").strip()
    if not primary:
        raise RuntimeError("PostgreSQL DSN is empty.")
    return psycopg.connect(primary, autocommit=autocommit)
...
def _normalize_manual_entry(raw: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(raw, dict):
        return None
    database_url = _canonicalize_manual_postgres_dsn(str(raw.get("database_url") or "").strip())
...
def _discover_postgres_schemas(
    ...
    *,
    ...
    if psycopg is None:
        return []
    try:
        conn = _connect_postgres(default_dsn, autocommit=True)
    except Exception:
        return []
        ...
        if key in merged:
            ...
            if item.get("backend") == "postgres":
                ...
                if _is_postgres_dsn(dsn) and psycopg is not None:
                    try:
                        conn = _connect_postgres(dsn, autocommit=True)
                        try:
...
def delete_mmp_database(database_id: str, *, drop_data: bool = True, include_stats: bool = False) -> Dict[str, Any]:
    ...
    if drop_data:
        ...
        conn = _connect_postgres(database_url, autocommit=True)
        try:
            with conn.cursor() as cursor:
