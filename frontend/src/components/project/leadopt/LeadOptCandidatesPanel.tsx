...
interface LeadOptCandidatesPanelProps {
  ...
  emptyStateContext?: Record<string, unknown>;
}
...
export function LeadOptCandidatesPanel({
  ...
  const renderPredictAction = (
    ...
    const isActionDisabled = isPending || !referenceReady || !smiles;
    const buttonClass = [
    ...
    >
      ...
      {renderedRows.length === 0 ? (
        <p className="muted small">
          {cardMode ? 'No scored candidates yet.' : readText(emptyStateHint).trim() || 'No results yet.'}
        </p>
      ) : (
        cardMode ? (
