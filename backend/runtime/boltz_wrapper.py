...
def _download_with_fallback(
    *,
    ...
    """
    Download file with retry/fallback to reduce transient network failures.
    """
    candidates = _unique_url_list(urls)
    if not candidates:
        raise RuntimeError(f"No download URL provided for {label}.")

    try:
        retries = int(os.environ.get("BOLTZ_DOWNLOAD_RETRIES", "3"))
    except ValueError:
        retries = 3
    ...
    for url in candidates:
        for attempt in range(1, retries + 1):
            try:
                click.echo(f"Downloading {label} from {url} (attempt {attempt}/{retries})")
                urllib.request.urlretrieve(url, str(destination))  # noqa: S310
                if destination.exists() and destination.stat().st_size > 0:
                    return
                raise RuntimeError(f"{label} downloaded but file is empty: {destination}")
    ...
    raise RuntimeError(f"Failed to download {label} from all URLs. Last error: {last_error}") from last_error
...
def download_boltz1(cache: Path) -> None:
    ...
    if not ccd.exists():
        ...
        _download_with_fallback(
            urls=[os.environ.get("BOLTZ_CCD_URL", ""), CCD_URL],
            destination=ccd,
            label="CCD dictionary",
        )
    ...
    if not model.exists():
        ...
        _download_with_fallback(
            urls=[os.environ.get("BOLTZ1_MODEL_URL", ""), *BOLTZ1_URL_WITH_FALLBACK],
            destination=model,
            label="Boltz-1 checkpoint",
        )
...
def download_boltz2(cache: Path) -> None:
    ...
    if not tar_mols.exists():
        ...
        _download_with_fallback(
            urls=[os.environ.get("BOLTZ2_MOLS_URL", ""), MOL_URL],
            destination=tar_mols,
            label="Boltz-2 mols.tar",
        )
    if not mols.exists():
        click.echo(
    ...
    if not model.exists():
        ...
        _download_with_fallback(
            urls=[os.environ.get("BOLTZ2_MODEL_URL", ""), *BOLTZ2_URL_WITH_FALLBACK],
            destination=model,
            label="Boltz-2 checkpoint",
        )
    ...
    if not affinity_model.exists():
        ...
        _download_with_fallback(
            urls=[os.environ.get("BOLTZ2_AFFINITY_MODEL_URL", ""), *BOLTZ2_AFFINITY_URL_WITH_FALLBACK],
            destination=affinity_model,
